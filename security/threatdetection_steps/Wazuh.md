...
or Wazuh Server) is deployed in a Docker instance in an existing node. Several devices on the network will have the agent installed and configured to send logs back to the manager.

> ### Design Limitations.
>Placing the server  in the same path as other Docker instances and VM's means any vulnerability to the baremetal or network itself also affects Wazuh and its
...
-----------------
### Goals
- [x] Install Wazuh Manager on Docker instance
- [x] Install Agent(s) on Windows OS
